...
let neverWonTabIeBody = document.getElementById("neverWonTableBody")
team_in.addEventListener("keyup",function(e)
 {
    if (e.key === 'Enter'){
        let team = team_in.value;
    
    if (nbaTeams.won.hasOwnProperty(team)){
        addtoTable(team, wonTableBody);
        console.log(team);
    }
    else if (nbaTeams.neverWon.includes(team)){
        addtoTable(team, neverWonTabIeBody);
    }
    else {
...
function addtoTable(team, table){
